...
class ConversationSummary(BaseModel):
    ...
    last_message: str    # snippet of the last user message
    turn_count: int
...
async def list_conversations(user_id: str = Depends(verify_token)):
    ...
    try:
        ...
        # Pull all turns across all conversations
...
async def get_conversation(conversation_id: str, user_id: str = Depends(verify_token)):
    """
    Retrieve all turns for a specific conversation so the frontend
    can restore the chat session.
    """
    try:
        vector_store = get_vector_store()
        turns = vector_store.get_conversation_history(
            ...
            last_n=9999,  # get everything for full restore
        )
